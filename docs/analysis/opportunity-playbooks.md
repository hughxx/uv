...
更新：2026-09-14。状态：目标规格。基础注册库、联合候选比较、阶段进度与事件重评已实现；完整预测、任务SOP和热更新仍未实现。

## 1. 核心目标

识别对手位置、行动或布局中的机会，在有效窗口内调用标准解法；根据新观测修正判断、切换打法，核对实际收益并安排后续转化。新打法通过独立定义和注册加入，不重写中央决策循环。

选择单位是可修订的跨回合计划，而非单步最高分动作。持续保留真实进度和资源，不无条件坚持原来的胜势判断；未预计的反制可以使当前计划失效，并由另一套打法接管。机会既可来自对手失误，也可来自任务SOP、宝藏时机和价格规律。
...
例如一个待验证的转化链可以包含“抢到唯一资源 → 确认金币到账 → 安排购买与运送升级券 → 在安全时机升级 → 腾出角色做更高收益任务”。后续步骤须依据真实状态推进，不因预计会得钱就提前购物。
...
每个阶段声明预期增量、比较基线、兑现期限和观测凭据，不能因匹配成功就给优势计数器加一。分别记录已确认的金币／积分／建筑等变化，以及尚未兑现的路线、时间窗口与胜率估计。相对对手的收益若不可见，应保留未知，不能只因自己金币增加就判定领先扩大。
...
基础实现已有条件变化、阶段交接及资源校验的回归测试；收益最优性、真实对局效果、目标环境时限和热更新仍需独立验证。当前计算量较小，检测器逐回合重评，尚未实现按依赖精确筛选事件订阅者。
