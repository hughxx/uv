...
用户已进一步明确最重要的目标：识别对手破绽后，迅速调用针对性打法并持续扩大优势；新打法能快速增补。因此核心扩展入口应是“机会识别器 + 战术方案库Playbook + 跨回合执行与优势转化”，中央分配器为它们协调资源。完整契约见[opportunity-playbooks.md](opportunity-playbooks.md)。当前仍是设计文档，尚不具备已实现的实时策略或热更新功能。
...
对外循环保留少量清晰接口：`observe(request)`、`propose(context)`、`allocate(candidates)`、`compile(plan)`。内部propose要支持机会检测和整套Playbook候选，allocate要支持阶段需求与持续承诺；不能把这些接口默认理解成仅比较单步动作。这是接口设计说明，本轮不创建相应代码文件。

本轮结合[同伴分析评审](peer-review.md)细化收益矩阵、风险地图及短期路径预留，整体架构方向保持不变。分层是职责边界，不要求每层独立服务或先建立大量空目录。

相较同伴评审时的方案，本次将战术方案库提升为核心能力。已知模式可在观察到的当前回合触发；赛间新增打法不修改主循环；对局中换定义版本则单独受比赛环境与版本迁移约束，不将这三种能力混称为“实时支持”。
...
Playbook必须包含触发条件、可利用的行动、常见反制、收益兑现和后续转化。其一次执行PlaybookRun产生下面的Jobs；Job可以承接ChallengeSession，但不能替代战术执行状态。初始进入条件、持续安全条件、阶段完成条件和退出条件分别定义，避免把“对手被迫回防”这种预期进展误认成整个打法失效。

持有计划意味着保留后续收益与切换成本，不意味着无视新局面。统一比较整套路线的剩余价值；危险升级、对手反制成功或窗口失效时按分支改道／中止，释放尚未消耗的预留资源。

事件驱动的滚动重评是主循环契约：先确认实际结果，再从完整观测差分、时间推进和预期落空生成事件，修正依赖它们的机会与胜势判断，然后统一比较“续打／内部变招／换另一套打法／恢复基线”。每次仅提交当前回合动作，不预先锁定未来命令。新打法不必嵌在旧打法的分支里；交接保留真实位置、资产和任务状态，按当前起点估算剩余收益及转换代价。

事件处理器只更新证据或提出候选，同轮事件批量汇入同一快照和协调入口，不各自抢写动作。活跃方案每轮检查安全条件、时间窗口及证据有效期；实质失效绕过普通防抖，小幅估值波动则避免反复横跳。预期优势和已兑现成果分别记账，对手出现新反制时允许撤销此前的高置信判断。详细契约及交接示例见Playbook文档§7。
