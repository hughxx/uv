...
2026-09-16真实Demo盘面审计发现：隐式空等与动作风险基线不一致、局部威胁遗漏、双方浪潮估值和任务期限耦合尚有缺陷。下述现有实现不代表这些问题已解决，具体证据与优先级见[自研审计](framework-review.md)，任务／模型／新闻接入见[实用说明](task-and-news-guide.md)。
...
回防期限补齐了“已有炮位不流失”之外的另一层：白天按工人数安排返程，夜间加入没有进行中任务的开拓者；需求不超过现存武器数。使用当前障碍图到各炮合法邻格的反向最短距离场，做一人一炮的联合匹配，白天预留两轮站位余量。默认比较顺序为预测角色死亡数、近敌压力下丢失的已有炮位、不可匹配数量、超期路径步数之和、普通效用；有冗余炮手时仍可交易或联合换岗。`Candidate.return_commitments`表达完成某条路线后的站位和总耗时，目前卖矿接入完整的“去商人—卖出—返岗”成本，其他动作只计下一站位。这样不会仅因交易第一步恰好朝基地走，就把赶不及的整趟交易判为安全。

该策略是可关闭的保守基线（`timely_defense_return`），不是最优部署证明：夜间优先补足可用炮位会牺牲部分任务／交易窗口；两轮余量也尚未实战标定。友方角色在距离场中按可协同让位处理，未来路线没有无碰撞保证，当前步仍交联合校验。敌人移动、击杀、建筑变化每轮重算，不用静态射程圈假装安全。距离场计算上限120ms且不超过剩余预算的一半，未完成时明确记录`budget`并退回其他约束，不把缺少结果当成零路径；异常规模（超过3炮或8操作员）跳过该模型并报告`size-limit`。不自动中断已接任务，也不能据此承诺第一夜存活。

短期保护使用`ThreatEnvelope`：暂按可见敌对机器人移动一格并攻击一次估算，不因预计击杀或墙体就消除反击。调度先最小化该模型下角色死亡数，再比较炮位、回防期限及普通效用；`RuleProfile.guard_projected_role_deaths`可关闭这项策略假设。保护不等于完整基地生存约束，也不证明机器人必定选择该角色。低血药剂、减小暴露的合法撤步会参与同一次联合搜索；任务中的开拓者只能在可确认的同一驻留区内避让，不自动承担未知退任务代价。
